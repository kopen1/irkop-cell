...
test('hash: format tetap pbkdf2$v1$iterations$salt$hash dengan 210000 iterasi', async () => {
  const stored = await hashPassword('secret1234');
  const parts = stored.split('$');
  assert.equal(parts.length, 5);
  assert.equal(parts[0], 'pbkdf2');
  assert.equal(parts[1], 'v1');
  assert.equal(parts[2], '210000');
...
test('konsistensi lintas-runtime: hash PBKDF2 standar (crypto Node) tetap valid', async () => {
...
test('password < 8 karakter ditolak; iterasi 210000 selesai dalam batas wajar', async () => {
  ...
  assert.ok(elapsed < 15000, `210k iterasi terlalu lama: ${elapsed}ms`);
});
