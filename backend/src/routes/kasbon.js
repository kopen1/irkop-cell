...
import { readBody, asInt, asDate } from '../lib/validate.js';
...
export async function createKasbon(db, request, ctx) {
  ...
  return { id: res.lastRowId, nominal, status: 'belum_lunas', tanggal };
}
...
export async function updateKasbon(db, request, ctx, idStr) {
  ...
  if (body.status === 'lunas' && old.status !== 'lunas') {
    ...
    await db.exec(
      ...
      sesi.id, akun, old.nominal, id, key, nowIso()
    );
  ...
  return { id, message: 'Kasbon diperbarui' };
}