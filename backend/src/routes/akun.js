...
export async function createAkun(db, request, ctx) {
  ...
  if (!['tunai', 'bank', 'e_wallet', 'digital', 'lainnya'].includes(tipe)) {
    throw err(400, 'invalid_value', 'tipe akun tidak valid');
  }
  ...
  const res = await db.exec(
    'INSERT INTO akun_master (nama_akun, tipe, aktif, created_at) VALUES (?, ?, 1, ?)',
    namaAkun, tipe, nowIso()
  );
